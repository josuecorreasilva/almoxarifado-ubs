...
with st.sidebar:
    ...
    elif perfil_escolhido == "ALMOXARIFADO":
        st.session_state.perfil = "ALMOXARIFADO"
        st.session_state.ubs_nome = "Almoxarifado Central"
        st.success("Logado como: **Almoxarifado**")
    else:
        st.session_state.perfil = "GESTAO"
        st.session_state.ubs_nome = "Gestão Central"
        st.warning("Logado como: **Gestão (Acesso Total)**")

    st.markdown("---")
    st.markdown("💡 *Arquitetura 100% integrada ao Supabase.*")
...
# --- CARREGAMENTO DE MATERIAIS DO SUPABASE (Substitui Drive) ---
...
with aba1:
    ...
    with col_distrito:
        st.selectbox("Distrito", [distrito_detectado], disabled=True)
    with col_ubs:
        st.selectbox("Unidade Requisitante", [st.session_state.ubs_nome], disabled=True)
    ...
    # Puxa o valor unitário da tabela de materiais
    ...
    if len(st.session_state.carrinho) > 0:
        ...
        for i, item in enumerate(st.session_state.carrinho):
            ...
            c4.write(f"**Subtotal:** R$ {item['subtotal']:.2f}" if st.session_state.perfil in ["GESTAO", "ALMOXARIFADO"] else "")
...
# ==============================================================================
# --- ABA 2: ALMOXARIFADO & ENTRADA DE LOTES (Perfil Almoxarifado / Gestão) ---
# ==============================================================================
    ...
    with aba2:
        ...
        try:
            ...
            if res_estoque_atual.data:
                ...
                st.info("Nenhum lote registrado no estoque central no momento.")
        except Exception as e:
            st.error(f"Erro ao carregar estoque: {e}")

# --- ABA DE CONSULTA DE ESTOQUE (Caso perfil UBS) ---
...
if st.session_state.perfil in ["ALMOXARIFADO", "GESTAO"]:
    with aba4:
        ...
        if not df_materiais.empty:
            ...
            st.info("O catálogo está vazio.")import streamlit as st
import pandas as pd
from datetime import datetime, date
import time
import os
